...
async function promptForFirstRunConfig(envFile, existingEnv) {
  ...
  try {
    ...
    // Write minimal .env for DATABASE_URL (not user-facing)
    const dbUrl = existingEnv.DATABASE_URL
      || `file:${path.join(settingsDir, "data", "structureclaw.db").replace(/\\/gu, "/")}`;
    const lines = [
      `# StructureClaw internal config - generated by sclaw doctor`,
      `DATABASE_URL=${dbUrl}`,
      ``,
    ];
...
async function invokeDoctor(rootDir, env) {
  ...
  if (isInstalled) {
    ...
    // Interactive first-run wizard if no settings.json AND no .env exist
    if (!runtime.pathExists(path.join(path.dirname(paths.envFile), "settings.json"))
        && !runtime.pathExists(paths.envFile)) {
