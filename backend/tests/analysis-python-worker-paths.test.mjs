...
describe('analysis python worker import paths', () => {
  test('should include structure protocol and sibling skill roots from the actual src tree', () => {
    ...
    expect(worker).toContain('AGENT_SKILLS_ROOT = CURRENT_DIR.parents[2]');
    expect(worker).toContain('SRC_ROOT = CURRENT_DIR.parents[3]');
    ...
    expect(worker).not.toContain('SKILL_ROOT = CURRENT_DIR.parents[2]');
  });
});
