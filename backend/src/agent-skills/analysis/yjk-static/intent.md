...
- `zh`: 当用户要求使用 YJK（盈建科）进行结构静力计算、设计验算时使用。自动将 V2 模型转换为 YJK 格式，启动盈建科软件执行建模、前处理、整体计算，并在计算完成后抽取节点位移、构件内力、包络和楼层统计等结构化结果。需要已安装 YJK 8.0 并配置 `YJKS_ROOT` 或 `YJK_PATH` 指向安装根目录；BIT/在线授权安装会自动优先通过 `YjkLauncher.exe` 启动并发送 `yjksipccontrol` 后附着控制，`YJK_DIRECT_RUN=1` 可强制使用 SDK 直启。若只想启动计算不等待，可设置 `YJK_START_ONLY=1` 或 `YJK_ASYNC_CALC=1`。
- `en`: Use when the request asks for YJK-based structural static analysis or design checks. Automatically converts the V2 model to YJK format, launches YJK for modeling, preprocessing, and full calculation, then extracts structured node displacements, member forces, envelopes, and floor statistics after completion. Requires YJK 8.0 and `YJKS_ROOT` or `YJK_PATH` pointing to the install root; BIT/online-authorized installs automatically prefer `YjkLauncher.exe`, send `yjksipccontrol`, and attach to the launched session. Set `YJK_DIRECT_RUN=1` to force SDK direct launch. Set `YJK_START_ONLY=1` or `YJK_ASYNC_CALC=1` only when starting the calculation without waiting is desired.
